...
export default function RoomDetailModal({ isOpen, onClose, room }: RoomDetailModalProps) {
    ...
    const nextImg = () => {
        setCurrentImgIdx((prev) => (prev === room.images.length - 1 ? 0 : prev + 1));
    };

    const prevImg = () => {
        setCurrentImgIdx((prev) => (prev === 0 ? room.images.length - 1 : prev - 1));
    };
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-center">
                    ...
                    <div className="flex items-center justify-center gap-4 md:col-span-7">
                        <button
                            onClick={prevImg}
                            className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-gray-700 hover:bg-gray-50 transition-colors focus:outline-none"
                        >
                            <LuChevronLeft className="w-5 h-5 stroke-[2]" />
                        </button>
                        <span className="text-sm font-medium text-gray-800 select-none tracking-wide">
                            {currentImgIdx + 1}/{room.images.length}
                        </span>
                        <button
                            onClick={nextImg}
                            className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-gray-700 hover:bg-gray-50 transition-colors focus:outline-none"
                    ...
                    <div className="md:col-span-5">
                        <button className="w-full bg-[#4a0a0c] hover:bg-[#a62121] text-white py-3 px-8 rounded-full font-bold text-sm uppercase tracking-widest transition-colors shadow-sm duration-200">
                            Book Now
                        </button>
                    </div>
