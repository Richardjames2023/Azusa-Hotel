...
export default function RoomCard({ room, onReadMore }: RoomCardProps) {
    ...
    return (
        <div
            className="bg-white rounded-lg overflow-hidden border border-gray-200/60 shadow-sm flex flex-col justify-between group/card"
                ...
                <div className="p-5 lg:p-6 pb-2">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        {room.specs.map((spec, sIdx) => (
                            <span
                                key={sIdx}
                                className="bg-gray-100 text-gray-600 text-xs px-2.5 py-1 rounded border border-gray-200/40"
                            >
                                {spec}
                            </span>
                        ))}
                    ...
                    <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-3 text-gray-500 min-h-[32px] items-center">
                            {room.amenities.map((item, iIdx) => {
                                ...
                                return (
                                    <div
                                        key={iIdx}
                                        title={item.label}
                                        className="hover:text-gray-900 transition-colors cursor-help"
                                    >
