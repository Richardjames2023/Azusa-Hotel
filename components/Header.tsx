...
export const Header: React.FC = () => {
  ...
  const [activeCalendarSelector, setActiveCalendarSelector] = useState<
    "in" | "out" | null
  >(null);
  const [currentCalendarView, setCurrentCalendarView] = useState<Date>(
    new Date(2026, 3, 1),
  ); // Default view set to April 2026
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      if (
        ...
        const isHamburgerClick = (event.target as HTMLElement).closest(
          ".hamburger-trigger",
        );
  ...
  return (
    <header className="w-full bg-white font-sans selection:bg-amber-200">
      ...
      <div
        className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] transition-opacity duration-300 ${showDrawer ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      >
        ...
        >
          <div>
            ...
            <nav className="flex flex-col text-sm tracking-wide font-medium">
              ...
              <span className="text-[10px] text-[#D4AF37]/70 uppercase tracking-widest font-bold block mb-2 px-4">
                Explore More
              </span>
      ...
      <div className="w-full max-w-[1440px] mx-auto px-6 h-[72px] flex items-center justify-between relative z-50 bg-white">
        ...
        <nav className="hidden lg:flex items-center space-x-6 text-[13px] font-bold text-black tracking-tight">
          <Link href="/rooms" className="hover:opacity-70 transition-opacity">
            Rooms
          </Link>
          <Link
            href="/restaurant"
          ...
          <div className="relative z-50" ref={moreMenuRef}>
            ...
            >
              <span>More</span>
              <svg
                className={`w-3 h-3 text-black stroke-[2.5] transition-transform duration-200 ${showMoreMenu ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div
              className={`absolute left-0 mt-3 w-48 bg-white border border-gray-100 rounded-xl shadow-2xl py-1.5 transform transition-all duration-200 origin-top-left ${showMoreMenu ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}
            >
              <Link
                href="/about"
                className="block px-4 py-2.5 text-xs text-gray-700 hover:bg-gray-50 hover:text-black font-semibold transition-colors"
              >
                About Us
              </Link>
              <Link
        ...
        <div className="flex items-center space-x-3 relative z-50">
          <div className="relative" ref={localizationRef}>
            ...
            {/* <div className="absolute right-0 mt-3 w-64 bg-white border border-gray-100 rounded-xl shadow-2xl p-4 grid grid-cols-2 gap-4 transform transition-all duration-200 origin-top-right opacity-100 scale-100 translate-y-0">
            <div>
              <span className="block text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-2">Language</span>
              <div className="block w-full text-left text-xs py-2 px-2.5 rounded-lg font-semibold bg-[#4A0A15] text-white select-none">
                {currentLang}
              </div>
            </div>
      ...
      <div className="w-full bg-[#DCB286] py-3.5 px-6 relative z-10">
        <div className="max-w-[1320px] mx-auto bg-white rounded-lg shadow-md flex flex-col md:flex-row items-center divide-y md:divide-y-0 md:divide-x divide-gray-200 p-1 relative">
          ...
          <div
            className="w-full md:w-5/12 flex items-center px-4 py-2.5 relative"
            ref={roomRef}
          >
            <div className="text-gray-500 mr-3">
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M2 4v16M2 8h20M2 12h20M22 4v16M6 12V8M10 12V8"
                />
              </svg>
            </div>
            ...
            <div
              className={`absolute top-full left-0 w-full bg-white mt-2 border border-gray-100 rounded-xl shadow-2xl py-1.5 z-40 transform transition-all duration-200 origin-top ${showRoomSuggestions ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
            >
              {sampleRoomTypes
          ...
          <div
            className="w-full md:w-6/12 grid grid-cols-2 divide-x divide-gray-200 relative"
            ref={calendarRef}
          >
            <div
              onClick={() => {
                setActiveCalendarSelector("in");
                setShowRoomSuggestions(false);
              }}
            ...
            >
              <div className="flex items-center">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[13px] text-gray-800 font-bold mt-0.5">
                    {formatDisplayDate(checkInDate, "Tue 14 Apr")}
                  </span>
                </div>
              </div>
              <svg
            ...
            >
              <div className="flex items-center">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[13px] text-gray-800 font-bold mt-0.5">
                    {formatDisplayDate(checkOutDate, "Wed 15 Apr")}
                  </span>
                </div>
              </div>
              <svg
            ...
            <div
              className={`absolute top-[110%] left-0 w-full md:w-[460px] bg-white border border-gray-100 rounded-xl shadow-2xl p-4 z-50 transform transition-all duration-200 origin-top-left ${activeCalendarSelector ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}
            >
              <div className="flex items-center justify-between mb-4 px-1">
                <span className="text-xs font-bold text-gray-800 uppercase tracking-wider">
                  {currentCalendarView.toLocaleString("en-US", {
                    month: "long",
                    year: "numeric",
                  })}
                </span>
                  ...
                  <button
                    onClick={() =>
                      setCurrentCalendarView(
                        new Date(
                          currentCalendarView.getFullYear(),
                          currentCalendarView.getMonth() - 1,
                          1,
                        ),
                      )
                    }
              ...
              <div className="grid grid-cols-7 gap-1">
                {getDaysInMonthArray(currentCalendarView).map(
                  (dayDate, index) => {
          ...
          <div className="w-full md:w-2/12 p-1">
            <button
              ...
              className="w-full bg-[#4A0A15] text-white py-3 px-6 rounded-md font-bold text-[13px] tracking-wide hover:bg-[#36070E] transition-colors whitespace-nowrap"
            >
              Search
            </button>
          </div>
